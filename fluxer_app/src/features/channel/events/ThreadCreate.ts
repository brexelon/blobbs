...
export function handleThreadCreate(data: Channel, _context: GatewayHandlerContext): void {
	...
	// Initialize the thread's read state exactly like a regular channel create does.
	// A thread carries its own channel id, so without this its unread/mention counts
	// never feed the community (guild) badge — a mention in the thread would not keep
	// the server badge lit until the thread is opened, unlike a text channel.
	ReadStates.handleChannelCreate({channel: data});
